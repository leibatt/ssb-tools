...
COPY INTO supplier from '${data_folder}/sf_${scale_factor}/supplier.tbl' USING DELIMITERS '|', '|\n';
  ...
  cursor.execute("copy customer from '"+data_file+"' header ")
  ...
  cursor.execute("copy date_ from '"+data_file+"' header ")
  ...
  cursor.execute("copy part from '"+data_file+"' header ")
  ...
  cursor.execute("copy supplier from '"+data_file+"' header ")
  ...
  cursor.execute("copy lineorder from '"+data_file+"' header ")

if __name__ == "__main__":
  if len(argv) == 2:
    duckdbConfigPath = argv[1]
    ssbConfigPath = argv[2]
  try:
    duckdbConfig = json.load(open(duckdbConfigPath))
    ssbConfig = json.load(open(ssbConfigPath))
    dbFilename = duckdbConfig['dbFilename']
    dataFolder = os.path.join(ssbConfig["data-folder"],"sf_"+ssbConfig["scale-factor"])
